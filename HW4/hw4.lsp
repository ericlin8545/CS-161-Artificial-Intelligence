...
; Color every possible c on the node n
(defun at-least-one-color (n c k)
  (cond ((= c k) (cons (node2var n c k) nil))
...
; Reqire each pair of variables to have at least one false variable.
(defun at-most-one-color-help (n c g k)
  (cond ((= g (- k 1)) (cons (list (- (node2var n c k)) (- (node2var n (+ g 1) k))) nil))
        (t (cons (list (- (node2var n c k)) (- (node2var n (+ g 1) k))) (at-most-one-color-help n c (+ g 1) k)))
  )
)
...
(defun at-most-one-color (n c k)
  (cond ((= c k) nil)
        ((= c (- k 1)) (at-most-one-color-help n c c k))
        (t (append (at-most-one-color-help n c c k) (at-most-one-color n (+ c 1) k)))
  )
)
...
; Contatenate the results of at-least-one-color and at-most-one-color
...
; Go through every color to ensure the color will not be used in both ends of the edge e.
(defun generate-edge-clauses-help (e c k)
  (cond ((= c k) (cons (list (- (node2var (first e) c k)) (- (node2var (second e) c k))) nil))
        (t (cons (list (- (node2var (first e) c k)) (- (node2var (second e) c k))) (generate-edge-clauses-help e (+ c 1) k)))
  )
)

(defun generate-edge-clauses (e k)
  (generate-edge-clauses-help e 1 k)
)
